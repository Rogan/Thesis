...
% \cite{Enright1992} collocation method - optimal? direct transcription with Conway
%\textcite{Enright1992} hermite simpson/RK integration, direct shooting/multiple shooting/collocation, concludes -- direct transcription, NLP, \emph{parallel shooting} RK. collocation, determining a single minimum fuel, low thrust, earth-moon transfer.
%future work - lower thrust levels, lower initial and final orbits, non-coplanar transfers
%size of problem - much better mesh distribution, exploit sparsity
...
% Something about objective functions, to lead in to...
This highlights that selection of an objective function can be a difficult task in itself. Many studies instead search for Pareto-optimality, based on the economic theory by Italian Vilfredo Pareto. Pareto-optimisation is a search for the best solution that can be found without compromising other objectives, in this case the best fuel efficiency that can be found without taking an unreasonable amount of time \parencite{Lee2005, Coverstone2000}.
...
% unfortunately CGA in GESOP not up to scratch, use SOCS for higher-order problems
