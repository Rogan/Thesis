...
Due to the direct shooting inherent in the SOCS algorithm, initial guesses are not required for all the states at each grid point during optimisation. However, if the trajectory is simulated using a multiple shooting method, the algorithm propagates the state forwards independently from each of these grid points. Fortunately the multiple shooting algorithm implemented in GESOP includes automatic initial guess estimation for the grid points.
